...
def title_for(t: str) -> str:
    title = f"{t} in Vellore | Vellore Catering World"
...
def make_faqs(item, ctx, seed):
    ...
    return [
        ...
        ("Can I enquire directly on WhatsApp?", f"Yes. Use the enquiry form on this page; it sends your name, contact number, event date and {t.lower()} request to the Vellore Catering World WhatsApp number."),
...
def render_page(item, catalog):
    ...
    body=f'''<!DOCTYPE html>
<!-- Vellore Catering World | {item['output']} | GENERATED V2 from uploaded Catering Types workbook -->
<html lang="en-IN">
<head>
  <!-- @head:start -->
  <!-- @head:end -->
...
<body data-wa="{CONFIG['whatsapp']}" data-page-key="{E(slug)}">
  ...
  <main id="main">
    ...
    <!-- 24 Google Reviews -->
    <section class="band" id="google-reviews"><div class="container">{section_head('Google reviews',f'Check Vellore Catering World on Google','The website does not hard-code a star rating or review count because those values change.')}<p class="center-copy">Use the live Google result to view current public feedback before booking.</p><div class="btn-row btn-row--center"><a class="btn btn--ghost" href="https://www.google.com/maps/search/?api=1&amp;query={quote('Vellore Catering World, '+CONFIG['street']+', Vellore '+CONFIG['pin'])}" target="_blank" rel="noopener">Open Google Maps</a></div></div></section>
    ...
    <section class="band" id="contact"><div class="container">{section_head('Contact',f'Talk to Vellore Catering World about {t}','Call or WhatsApp for date availability and a menu discussion.')}<div class="contact-mini"><div><strong>Phone</strong><a href="tel:{CONFIG['phone_e164']}">{E(CONFIG['phone_display'])}</a></div><div><strong>WhatsApp</strong><a href="https://wa.me/{CONFIG['whatsapp']}" target="_blank" rel="noopener">{E(CONFIG['phone_short'])}</a></div><div><strong>Address</strong><span>{E(CONFIG['street'])}, Vellore, Tamil Nadu {E(CONFIG['pin'])}</span></div></div></div></section>

    <!-- 33 Google Map -->
    <section id="map"><div class="container">{section_head('Map & directions',f'Find our Vellore base','Open the map for directions; event service is coordinated separately to your venue.')}<div class="map-shell"><iframe title="Vellore Catering World location map" loading="lazy" referrerpolicy="no-referrer-when-downgrade" src="https://www.google.com/maps?q={quote(CONFIG['business_name']+', '+CONFIG['street']+', '+CONFIG['city']+' '+CONFIG['pin'])}&amp;output=embed"></iframe></div></div></section>
...
def render_other_page(catalog, groups):
    ...
    title="Other Catering Services in Vellore | Vellore Catering World"
    desc="Browse Vellore Catering World services by occasion, cuisine, service format, institution, festival, venue and dietary requirement, with direct WhatsApp enquiries."
    group_html=[]
...
def render_guide(slug,title,desc):
    ...
    items=topics[slug]
    meta=f'''<title>{E(title)} | Vellore Catering World</title><meta name="description" content="{E(desc)}"><link rel="canonical" href="{page_url(output)}"><meta name="robots" content="index,follow,max-image-preview:large">{jsonld(business_schema(),website_schema(),breadcrumb_schema([('Home','index.html'),('Guides','guides/index.html'),(title,output)]))}'''
...
def render_guides_index():
    ...
    meta=f'''<title>{E(title)} | Vellore Catering World</title><meta name="description" content="{E(desc)}"><link rel="canonical" href="{page_url(output)}"><meta name="robots" content="index,follow,max-image-preview:large">{jsonld(business_schema(),website_schema(),breadcrumb_schema([('Home','index.html'),('Guides',output)]))}'''
