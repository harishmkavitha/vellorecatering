...
def build_page(p, index, template):
    ...
    menu_link_html = (f'<a class="btn btn--ghost" href="{link(pre, ml["href"])}">{esc(ml["text"])}</a>'
                      if ml else "")
    share_text = f"Hello Vellore Catering, please share the {p['event_label'].lower()} menu and a quote."
    ...
    values = {
        ...
        "wa_hero": wa_link(f"Hello Vellore Catering, I would like a quote for {p['event_label'].lower()}."),
